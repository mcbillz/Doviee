...
function Sneakers() {
  const sneakersProducts = Products.filter((product) =>
    product.category.includes("sneakers")
  );
  ...
  const handleSearchInput = (event) => {
    ...
    const filteredResults = sneakersProducts.filter((item) =>
      item.name.toLowerCase().includes(query.toLowerCase())
    );
    setSearchResults(filteredResults);
  };

  const productsArray = searchQuery === "" ? sneakersProducts : searchResults;
  // PAGINATION
